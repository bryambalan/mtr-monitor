...
#DATABASE NAME
db_name = 'mtr'
#Credenciais com permissao de leitura/escrita na database
user = 'cabelo'
password = 'cabelo'
...
class HubEntry(SeriesHelper):
    class Meta:
        ...
        fields = ['time', 'loss', 'snt',  'last', 'avg', 'best', 'wrst', 'stdev', 'lat', 'lon']
...
def main():
    ...
    for hub in mtr_result['report']['hubs']:
        ...
        if len(hub['count']) < 2:
            hop = "0" + hub['count'] + ". " + hub ['ASN'] + " " + hub['host']
        else:
            hop = hub['count'] + ". " + hub ['ASN'] + " " + hub['host']
        HubEntry(
            time=report_time,
